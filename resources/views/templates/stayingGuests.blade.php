    ...
    <div class="col-xl-12">
                    ...
                    <div class="page-breadcrumb">
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item active" aria-current="page">Staying Guests</li>
                            </ol>
                        </nav>
                    </div>
                </div>
        ...
        <div class="row">
            <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                ...
                <form class="form-horizontal col-lg-12" role="form" method="POST" action="{{ url('/report-stayingGuests') }}" id="report_stayingGuests_form">
                            {!! csrf_field() !!}
                    
                    <div class="col-md-12">
                        <div class="card">
                            <div class="card-header">
                                <h4 class="mb-0">Search stayingGuestss</h4>
                            </div>
                            <div class="card-body">                                  
        ...
        <div class="card-body">
            <div class="table-responsive">
                <table id="report_table" class="table clientList table-striped table-bordered" style="width:100%">
                    ...
                    <tbody>
                        <?php $count=1 ?>
                    @foreach($data as $stayingGuests)
                            <tr class="table-success">
                                <td>{{ $count++ }}</td>
                                <td>{{ $stayingGuests->client_name }}</td>
                                <td>{{ $stayingGuests->room_title }}</td>
                                <td>{{ $stayingGuests->roomtype_title }}</td>
                                <td>{{ $stayingGuests->reservation_id }}</td>
                                <td>{{ $stayingGuests->adults }}</td> 
                                <td>{{ $stayingGuests->status }}</td> 
                                <td>{{ $stayingGuests->actual_stayingGuests }} - {{ $stayingGuests->stayingGuests }}</td> 
                                <td>{{ $stayingGuests->actual_stayingGuests }}</td> 
                            </tr>
                            @endforeach
